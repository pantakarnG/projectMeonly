...
import { getAuthorities,createAuthorities, updateStatusAuthorities, deleteAuthorities} from '../../../../service/Authorities.Service';
...
//   const [dataDepartment, setDataDepartment] = useState([]);
  ...
  async function fetchData(pageSize, currentPage, search, status) {
    let res = await getAuthorities(pageSize, currentPage, search, status);
    if (res) {
  ...
  function updateStatus(id, data) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let res = await updateStatusAuthorities(id, data);
        if (res) {
          if (res.statusCode === 200 && res.taskStatus) {
            Swal.fire({
              icon: 'success',
  ...
  function deleteData(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let res = await deleteAuthorities(id);
        if (res) {
          if (res.statusCode === 200 && res.taskStatus) {
            Swal.fire({
              icon: 'success',
